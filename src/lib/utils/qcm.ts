...
export function calculateNumberOfQuestions(recipientCount: number): number {
  ...
  if (recipientCount < 20) {
    return Math.max(10, Math.floor(recipientCount * 10 * 0.4)); // 50% du nombre de destinataires, minimum 10
  } else if (recipientCount < 50) {
    return Math.max(20, Math.floor(recipientCount * 10 * 0.3)); // 40% du nombre de destinataires, minimum 10
  } else if (recipientCount < 100) {
    return Math.max(30, Math.floor(recipientCount * 10 * 0.2)); // 30% du nombre de destinataires, minimum 20
  } else {
